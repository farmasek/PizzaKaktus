/* Assemble all pages for export */
export ManageIngredientsPage from './ManageIngredientsPage/index';
export ManagePizzasPage from './ManagePizzasPage/index';
export ManageCategoryPage from './ManageCategoryPage/index';
export ManageUsersPage from './ManageUsersPage/index';
export NotFoundPage from './NotFoundPage/index';
export LandingPage from './LandingPage/index';
export ShoppingCartPage from './ShoppingCartPage/index';
export MenuPage from './MenuPage/index';
export ManageOrdersPage from './ManageOrdersPage/index';
