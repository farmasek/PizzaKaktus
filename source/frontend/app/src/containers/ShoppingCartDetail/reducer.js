...
import { mapCart, mapCartItemData } from '../../models/CartItem';
...
const shoppingCartReducer = (state = new InitialState(), action) => {
  switch (action.type) {
    case FETCH_SHOPPING_CART: {
      const cart = mapCart(JSON.parse(localStorage.getItem('cart'))) ||
        new Map();
      return state.withMutations(s =>
        s.set('cart', cart).set('dialog', { showDialog: false }));
    }
    case ADD_TO_SHOPPING_CART: {
      ...
      cart = cart.set(
        i,
        mapCartItemData(action.pizza, fromJS(action.pizza.ingredientsId))
      );
    ...
    case CART_CUSTOMER_EDIT: {
      return action.field === 'email'
        ? state
            .setIn(['customer', action.field], action.value)
            .set('isLoading', true)
    ...
    case `${CART_CUSTOMER_EDIT}_prefill`: {
      return state
        .set('customer', mapCustomer(action.response))
        .set('isLoading', false);
    }
    ...
    case CHANGE_PIZZA_INGREDIENTS: {
      ...
      if (ingredients.includes(action.ingredientId)) {
        ingredients = ingredients.delete(
          ingredients.indexOf(action.ingredientId)
        );
      } else {
